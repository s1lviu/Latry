...
{
    qDebug() << "AudioEngine destructor - cleaning up safely";
    
    // Stop any ongoing operations immediately to prevent callbacks during destruction
    m_audioReady = false;
    m_recording = false;
    
    cleanupAudio();
    
    qDebug() << "AudioEngine destructor completed";
}
...
#if defined(Q_OS_IOS)
    // iOS VoIP apps benefit from additional prebuffering to handle:
    // - AVAudioSession interruptions (calls, notifications)
    // - iOS power management transitions
    // - Cellular/WiFi network switching
    // Use modest increase: 100ms over Android for iOS-specific resilience
    const int prebufFrames = (120 / FRAME_SIZE_MS); // 6 frames = 120ms for iOS
    qDebug() << "AudioEngine: Using iOS VoIP-optimized prebuffering: 120ms";
#else
    // Keep at least 80ms in buffer before playback for smooth operation
    const int prebufFrames = (80 / FRAME_SIZE_MS); // 4 frames = 80ms
#endif
...
void AudioEngine::setupAudio()
{
    if (!m_audioSink) {
...
#if defined(Q_OS_IOS)
        // iOS: Always align Qt format with iOS audio session to prevent quality issues
        double actualIOSSampleRate = ios_getActualSampleRate();
        double actualIOSBufferDuration = ios_getActualBufferDuration();
        qDebug() << "AudioEngine: iOS session actual rate:" << actualIOSSampleRate << "Hz, buffer:" << actualIOSBufferDuration << "s";
        
        // Force Qt to use the same sample rate as iOS audio session
        if (actualIOSSampleRate > 0 && actualIOSSampleRate != SAMPLE_RATE) {
            QAudioFormat iosAlignedFormat = outFormat;
            iosAlignedFormat.setSampleRate(static_cast<int>(actualIOSSampleRate));
            if (outputDevice.isFormatSupported(iosAlignedFormat)) {
                qDebug() << "AudioEngine: Aligning Qt format with iOS session:" << actualIOSSampleRate << "Hz";
                outFormat = iosAlignedFormat;
                resampler = std::make_unique<Resampler>(SAMPLE_RATE, outFormat.sampleRate(), CHANNELS);
            } else {
                qWarning() << "AudioEngine: iOS session rate" << actualIOSSampleRate << "Hz not supported by Qt";
            }
        }
#endif
...
#if defined(Q_OS_IOS)
            // iOS-specific: Try to match what iOS audio session actually configured
            qWarning() << "AudioEngine: iOS 16kHz format not supported by Qt device";
            
            // Try to match iOS session sample rate exactly
            if (actualIOSSampleRate > 0) {
                QAudioFormat iosFormat = outFormat;
                iosFormat.setSampleRate(static_cast<int>(actualIOSSampleRate));
                if (outputDevice.isFormatSupported(iosFormat)) {
                    qDebug() << "AudioEngine: Using iOS session sample rate:" << actualIOSSampleRate << "Hz";
                    outFormat = iosFormat;
                    if (outFormat.sampleRate() != SAMPLE_RATE) {
                        resampler = std::make_unique<Resampler>(SAMPLE_RATE, outFormat.sampleRate(), CHANNELS);
                    }
                } else {
                    qWarning() << "AudioEngine: iOS session rate" << actualIOSSampleRate << "Hz not supported by Qt, using preferred";
                    outFormat = outputDevice.preferredFormat();
                    if (outFormat.sampleRate() != SAMPLE_RATE) {
                        qWarning() << "AudioEngine: iOS will resample" << SAMPLE_RATE << "→" << outFormat.sampleRate() << "Hz (may affect quality)";
                        resampler = std::make_unique<Resampler>(SAMPLE_RATE, outFormat.sampleRate(), CHANNELS);
                    }
                }
            } else {
                qWarning() << "AudioEngine: Could not get iOS session rate, using device preferred format";
                outFormat = outputDevice.preferredFormat();
                if (outFormat.sampleRate() != SAMPLE_RATE) {
                    qWarning() << "AudioEngine: iOS will resample" << SAMPLE_RATE << "→" << outFormat.sampleRate() << "Hz (may affect quality)";
                    resampler = std::make_unique<Resampler>(SAMPLE_RATE, outFormat.sampleRate(), CHANNELS);
        ...
        // Log final audio configuration
        qDebug() << "AudioEngine: Final audio output format:" << outFormat.sampleRate() << "Hz," 
                 << outFormat.channelCount() << "channels," << outFormat.sampleFormat()
                 << "Resampler:" << (resampler ? "YES" : "NO");
        ...
        // Store objects created in audio thread FIRST
        ...
        {
            QMutexLocker locker(&m_audioStreamDeviceMutex);
            ...
            if (!m_audioStreamDevice->isOpen())
                m_audioStreamDevice->open(QIODevice::ReadOnly);
        }
        ...
        Q_ASSERT(m_audioStreamDevice && m_audioStreamDevice->isReadable());
...
#if defined(Q_OS_IOS)
    // iOS: Activate audio session during PTT to maintain background operation
    // This ensures the app stays active indefinitely while transmitting
    ios_activateAudioSession();
    qDebug() << "AudioEngine: iOS audio session activated for PTT background mode";
#endif
...
void AudioEngine::stopRecording()
{
    ...
    if (m_audioInputDevice) {
        ...
        m_audioInputDevice = nullptr; // Clear pointer to prevent access
    }
...
#if defined(Q_OS_IOS)
    // iOS: Recreate the audio sink atomically to restore routing without races in Qt's
    // QPlatformAudioSinkStream::createQIODeviceConnections. Keep the QIODevice alive.
    if (m_audioSink) {
        QAudioSink *oldSink = m_audioSink;
        m_audioSink = nullptr;

        auto recreateAndStart = [this, oldSink]() {
            // Stop/reset the old sink on its own thread without pumping the event loop.
            oldSink->stop();
            oldSink->reset();

            // Make a fresh sink (fresh platform stream)
            QAudioDevice outputDevice = QMediaDevices::defaultAudioOutput();
            QAudioSink *newSink = new QAudioSink(outputDevice, m_outputFormat, this);
            connect(newSink, &QAudioSink::stateChanged, this, [](QAudio::State st){
                qDebug() << "Audio sink state changed to" << st;
            });

            // Ensure we have a persistent, open device
            AudioStreamDevice *dev = nullptr;
            {
                QMutexLocker locker(&m_audioStreamDeviceMutex);
                if (!m_audioStreamDevice) {
                    m_audioStreamDevice = new AudioStreamDevice(&m_jitterBuffer,
                                                                m_outputResampler.get(),
                                                                m_outputFormat.sampleRate(),
                                                                m_outputFormat.sampleFormat(),
                                                                this);
                }
                if (!m_audioStreamDevice->isOpen())
                    m_audioStreamDevice->open(QIODevice::ReadOnly);
                dev = m_audioStreamDevice;
            }

            m_audioSink = newSink;

            // Start on the next event-loop turn to avoid reentrancy with teardown
            QMetaObject::invokeMethod(newSink, [newSink, dev](){
                Q_ASSERT(dev && dev->isReadable());
                newSink->start(dev);
            }, Qt::QueuedConnection);

            // Let the old sink be deleted asynchronously
            oldSink->deleteLater();
        };

        if (oldSink->thread() == QThread::currentThread()) {
            recreateAndStart();
        } else {
            QMetaObject::invokeMethod(oldSink, recreateAndStart, Qt::BlockingQueuedConnection);
        }
    }
...
void AudioEngine::processReceivedAudio(const QByteArray &audioData, quint16 sequence)
{
...
#if defined(Q_OS_IOS)
        // iOS: Handle potential packet reordering from cellular/WiFi switching
        if (diff > 32768) {
            // This looks like sequence wrap-around or late packet, not missing packets
            qDebug() << "AudioEngine: iOS packet reordering detected, seq:" << sequence << "expected:" << expected;
        } else
#endif
        if (diff > 0 && diff < 10) { // Reasonable missing packet count
            unsigned missing = diff;
            qDebug() << "AudioEngine: Missing" << missing << "packets, generating PLC";
            while (missing--) {
            ...
            // Too many missing packets - probably network issue, reset sequence tracking
            qWarning() << "AudioEngine: Too many missing packets (" << diff << "), resetting sequence tracking";
            m_jitterBuffer.clear(); // Clear potentially corrupted buffer
        }
    }
    ...
    if (decodedSampleCount > 0) {
        ...
        // Thread-safe pattern: Direct signal with object validity check
        {
            QMutexLocker locker(&m_audioStreamDeviceMutex);
            if (m_audioStreamDevice) {
                // Use Qt::QueuedConnection for thread safety but check object validity
                // The mutex ensures the object won't be deleted between check and call
                QMetaObject::invokeMethod(m_audioStreamDevice,
                                          &AudioStreamDevice::triggerReadyRead,
                                          Qt::QueuedConnection);
            }
        }
...
#if defined(Q_OS_IOS)
    // iOS-specific: Ensure audio sink is ready to receive new audio after flush
    // This prevents audio sink from staying in IdleState after buffer flush
    if (m_audioSink && m_audioStreamDevice) {
        QAudio::State currentState = m_audioSink->state();
        qDebug() << "AudioEngine: iOS audio sink state before flush recovery:" << currentState;
        
        if (currentState == QAudio::IdleState) {
            // Restart audio sink if it went idle
            m_audioSink->stop();
            QTimer::singleShot(50, this, [this]() {
                if (m_audioSink && m_audioStreamDevice) {
                    m_audioSink->start(m_audioStreamDevice);
                    qDebug() << "AudioEngine: iOS audio sink restarted after flush";
                }
            });
        }
    }
...
void AudioEngine::onAudioInputReadyRead()
{
...
#if defined(Q_OS_IOS)
    // Debug: Log audio format being used on iOS
    static bool formatLogged = false;
    if (!formatLogged) {
        qDebug() << "iOS Audio Format:" << m_inputFormat.sampleFormat() << "Sample Rate:" << m_inputFormat.sampleRate() << "Channels:" << m_inputFormat.channelCount();
        formatLogged = true;
    }
#endif
    ...
    if (m_inputFormat.sampleFormat() == QAudioFormat::Int16) {
        ...
        if (++debugCounterInt16 % 100 == 0) { // Every 100 frames (~2 seconds)
            float maxLevel = 0.0f;
            for (int i = 0; i < monoSamples; ++i) {
        ...
        for (int i = 0; i < monoSamples; ++i) {
            ...
            m_reusableFloatBuffer[i] = sample;
        }
#endif
        ...
        if (++debugCounterFloat % 100 == 0) { // Every 100 frames (~2 seconds)
            float maxLevel = 0.0f;
            for (int i = 0; i < monoSamples; ++i) {
                maxLevel = std::max(maxLevel, std::abs(m_reusableFloatBuffer[i]));
            }
...
void AudioEngine::cleanupAudio()
{
    ...
    // Stop and cleanup AudioStreamDevice safely to prevent access after destruction
    {
        QMutexLocker locker(&m_audioStreamDeviceMutex);
        if (m_audioStreamDevice) {
            // Disconnect any signals to prevent callbacks during destruction
            m_audioStreamDevice->disconnect();
            
            // Clear the pointer first to prevent new queued calls
            AudioStreamDevice* deviceToDelete = m_audioStreamDevice;
            m_audioStreamDevice = nullptr;
            
            // Release mutex before processing events to avoid deadlock
            locker.unlock();
            
            // Process any pending events that might reference the device
            // This ensures queued QMetaObject::invokeMethod calls are processed safely
            QCoreApplication::processEvents(QEventLoop::ExcludeUserInputEvents, 50);
            
            // Now safe to delete the device with thread-safe deleteLater()
            if (deviceToDelete->thread() == QThread::currentThread()) {
                deviceToDelete->deleteLater();
                QCoreApplication::sendPostedEvents(deviceToDelete, QEvent::DeferredDelete);
            } else {
                QMetaObject::invokeMethod(deviceToDelete, "deleteLater", Qt::QueuedConnection);
            }
        }
    }
