...
private:
    ...
    // Resampler may return more output than the sink can take in one go.
    // Keep the remainder here and serve it first on the next read().
    mutable QMutex m_tailMutex;
    std::vector<float> m_resampleTail;
    size_t m_tailPos = 0;
};
